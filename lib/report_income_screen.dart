...
class _ReportingScreenState extends State<ReportingScreen> {
  ...
  Future<void> _fetchData() async {
    await Future.wait(
        [_fetchCargos(), _fetchExpenses()]); // Fetch both in parallel
    setState(() {
      _isLoading = false;
    });
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (expense['BillOfLadingCost'] != null &&
                    double.tryParse(
                            expense['BillOfLadingCost']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'هزینه بارنامه',
                ...
                if (expense['HighwayToll'] != null &&
                    double.tryParse(
                            expense['HighwayToll']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'عوارض جاده ای',
                    expense['HighwayToll'],
                ...
                if (expense['DieselCost'] != null &&
                    double.tryParse(expense['DieselCost']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'هزینه گازوئیل',
                    expense['DieselCost'],
                ...
                if (expense['LoadingTip'] != null &&
                    double.tryParse(expense['LoadingTip']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'انعام بارگیری',
                    expense['LoadingTip'],
                ...
                if (expense['UnloadingTip'] != null &&
                    double.tryParse(
                            expense['UnloadingTip']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'انعام تخلیه',
                ...
                if (expense['DisinfectionCost'] != null &&
                    double.tryParse(
                            expense['DisinfectionCost']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'هزینه ضد عفونی',
                ...
                if (expense['OtherCost1'] != null &&
                    double.tryParse(expense['OtherCost1']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'هزینه های دیگر 1',
                    expense['OtherCost1'],
                ...
                if (expense['OtherCost2'] != null &&
                    double.tryParse(expense['OtherCost2']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'هزینه های دیگر 2',
                    expense['OtherCost2'],
                ...
                if (expense['OtherCost3'] != null &&
                    double.tryParse(expense['OtherCost3']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'هزینه های دیگر 3',
                    expense['OtherCost3'],
                ...
                if (expense['OtherCost4'] != null &&
                    double.tryParse(expense['OtherCost4']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'هزینه های دیگر 4',
                    expense['OtherCost4'],
                ...
                if (expense['OtherCost5'] != null &&
                    double.tryParse(expense['OtherCost5']?.toString() ?? '0') !=
                        0)
                  _buildCostRow(
                    'هزینه های دیگر 5',
                    expense['OtherCost5'],
                ...
                Container(
                  ...
                  child: _buildInfoRow('جمع کل هزینه', totalExpense,
                      isPrice: true, fontWeight: FontWeight.bold),
                ),
              ],
            ),
          ),
  ...
  Widget _buildCostRow(
      String label, dynamic value, String? imagePath, BuildContext context) {
    return Row(
      children: [
        Expanded(
          child: _buildInfoRow(label, value,
              isPrice: true, fontWeight: FontWeight.w500),
        ),
  ...
  double calculateTotalExpense(dynamic expense) {
    double billOfLadingCost =
        double.tryParse(expense['BillOfLadingCost']?.toString() ?? '0') ?? 0;
    double highwayToll =
        double.tryParse(expense['HighwayToll']?.toString() ?? '0') ?? 0;
    double dieselCost =
        double.tryParse(expense['DieselCost']?.toString() ?? '0') ?? 0;
    double loadingTip =
        double.tryParse(expense['LoadingTip']?.toString() ?? '0') ?? 0;
    double unloadingTip =
        double.tryParse(expense['UnloadingTip']?.toString() ?? '0') ?? 0;
    double disinfectionCost =
        double.tryParse(expense['DisinfectionCost']?.toString() ?? '0') ?? 0;
    double otherCost1 =
        double.tryParse(expense['OtherCost1']?.toString() ?? '0') ?? 0;
    double otherCost2 =
        double.tryParse(expense['OtherCost2']?.toString() ?? '0') ?? 0;
    double otherCost3 =
        double.tryParse(expense['OtherCost3']?.toString() ?? '0') ?? 0;
    double otherCost4 =
        double.tryParse(expense['OtherCost4']?.toString() ?? '0') ?? 0;
    double otherCost5 =
        double.tryParse(expense['OtherCost5']?.toString() ?? '0') ?? 0;

    return billOfLadingCost +
        highwayToll +
        dieselCost +
        loadingTip +
        unloadingTip +
  ...
  Widget _buildInfoRow(String label, dynamic value,
      {bool isPrice = false, FontWeight? fontWeight}) {
